<template>
  <div>
    <div class="case-list" v-show="moduleId" :style="{'height':winHeight+'px'}">
      ...
      >
        <a-table-column title="编号" data-index="cnumber" key="cnumber">
          <template slot-scope="text, record">
            <span class="case-link" @click="caseDetailFun(record)">{{record.cnumber}}</span>
          </template>
        </a-table-column>
        <a-table-column title="标题" data-index="title" key="title"></a-table-column>
        <a-table-column title="类型" data-index="ctype" key="ctype">
          <template slot-scope="text, record">{{getCaseType(record.ctype)}}</template>
        </a-table-column>
        <a-table-column title="优先级" data-index="priority" key="priority">
          <template slot-scope="text, record">{{caseSet.priority[record.priority]}}</template>
        </a-table-column>
        <a-table-column title="创建人" data-index="username" key="username"></a-table-column>
        <a-table-column title="创建时间" data-index="creation_time" key="creation_time"></a-table-column>
    ...
    <caseDetail v-model="showDetailDrawer" :caseData="caseDetailData"></caseDetail>
  </div>
</template>
<script>
  ...
  methods: {
    ...
    getCaseList(par = {}) {
      ...
      if (this.moduleId && this.moduleId != "all") {
        ...
        caseApi.getCaseByModule(this.moduleId, params).then(
          res => {
        ...
        params.title = this.searchData.title;
        params.user_ids = this.searchData.user_id.toString();
        caseApi.getCaseList(params).then(
          res => {
            const pagination = { ...this.pagination };
            pagination.total = res.data.total;
            this.caseList = res.data.data;
    ...
    sureDelFun() {
      caseApi.deleteCase({ id: this.editCaseId }).then(
        res => {
    ...
    exportCases() {
      ...
      caseApi.exportCase(params).then(
        res => {
    ...
    importCases() {
      ...
      caseApi.importCase(params).then(
        res => {
