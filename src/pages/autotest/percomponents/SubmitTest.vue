  ...
  <div class="submit-test">
    ...
    <ArrowContainer head-text="测试手机信息">
      <div slot="content" class="phone-info">
        <el-table :data="tableData" style="width: 100%;" max-height="250">
          ...
          <el-table-column prop="occupy" label="占用情况">
            <template slot-scope="scope">
              <el-tag
                size="mini"
                :type="scope.row.using ? 'warning' : 'success'"
              >{{scope.row.using ? '占用中' : '空闲'}}</el-tag>
            </template>
          </el-table-column>
        </el-table>
      </div>
    </ArrowContainer>
    <ArrowContainer head-text="测试用例信息">
      <div slot="content" class="phone-info">
        <el-table :data="testcaseList" style="width: 100%;" max-height="250">
          <el-table-column prop="id" label="ID"></el-table-column>
          <el-table-column prop="project" label="产品"></el-table-column>
          <el-table-column prop="case_name" label="名称"></el-table-column>
        </el-table>
      </div>
    </ArrowContainer>
    <ArrowContainer head-text="其他配置信息">
      <div slot="content" class="other-info">
        ...
        <p>
          是否安装包：
          <el-tag size="mini" type="success">{{isInstallApp ? '是' : '否'}}</el-tag>
        </p>
        <p>
          是否尝试登陆：
          <el-tag size="mini" type="success">{{jumpLogin ? '是' : '否'}}</el-tag>
        </p>
        <p>配置项：{{testConfig}}</p>
      </div>
    </ArrowContainer>
